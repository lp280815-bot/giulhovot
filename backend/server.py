...
async def get_processing_details(category: str):
    """Get detailed rows for a specific category (green, orange, purple, blue, special, command, emails, ka, ready_payment)."""
    if category not in ["green", "orange", "purple", "blue", "special", "command", "emails", "ka", "ready_payment"]:
...
async def export_ready_payment(request: ExportReadyPaymentRequest):
    ...
    try:
        ...
        for idx, row in enumerate(request.rows, 1):
            ...
            ws.cell(row=idx+1, column=6, value=row.get("invoice", ""))
            ws.cell(row=idx+1, column=7, value=calculate_payment_date(row.get("date", ""), row.get("payment_terms", "")))
            ws.cell(row=idx+1, column=8, value=row.get("details", ""))
